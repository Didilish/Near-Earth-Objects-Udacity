...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        As precondition, this constructor assumes that the collections of NEOs
        and close approaches haven't yet been linked - that is, the
        `.approaches` attribute of each `NearEarthObject` resolves to
        an empty collection, and the `.neo` attribute of
        each `CloseApproach` is None.

        However, each `CloseApproach` has an attribute (`._designation`) that
        matches the `.designation` attribute of the corresponding NEO. This
        constructor modifies the supplied NEOs and close approaches
        to link them together - after it's done,
        the `.approaches` attribute of each NEO has
        ...
        self.name_dict = dict()
        self.designation_dict = dict()

        for neo in self._neos:
            self.name_dict[neo.name] = neo
            self.designation_dict[neo.designation] = neo

    # NEOs and their close approaches linked together
    ...
    def query(self, filters=()):
        ...
        which isn't guaranteed to be sorted meaninfully, although is often
        sorted by time.
