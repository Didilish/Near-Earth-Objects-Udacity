...
class NearEarthObject:
    ...
    def __init__(self, designation, name, diameter=float('nan'),
                 hazardous=False):
    ...
    @property
    def fullname(self):
        """Return a representation of the full
        name of this NEO."""
        # fullname built using from self.designation and self.name.
    ...
    def __str__(self):
        ...
        if self.hazardous:
            return f"NEO {self.name} has a diameter of {self.diameter: .3f},\
...
class CloseApproach:
    ...
    def __str__(self):
        """Return `str(self)`."""
        return f"At {datetime_to_str(self.time)},\
        a CloseApproach is travelling to earth, distance of {self.distance},\
        au and velocity of {self.velocity} km/s "
